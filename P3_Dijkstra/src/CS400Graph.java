...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
	...
	protected class Path implements Comparable<Path> {
		 ...
		 */
		public Path(Path copyPath, Edge extendBy) { //extension constructor of Path 
			// TODO: Implement this constructor in Step 5.
			this.start = copyPath.start;  //the start of the path
			this.distance = copyPath.distance + extendBy.weight.doubleValue(); //the distance/ total cost will be the original distance added with the edge weight
			this.dataSequence = new LinkedList<>();
			for(int i = 0; i<copyPath.dataSequence.size(); i++)
			{
				this.dataSequence.add(copyPath.dataSequence.get(i)); 
			}
			this.dataSequence.add(extendBy.target.data);//adding the new path to the data sequence

			this.end = extendBy.target;  //finding the end vertex 
	...
	protected Path dijkstrasShortestPath(NodeType start, NodeType end) {      
		if(start == null || end == null || vertices.get(start ) == null || vertices.get(end) == null) //testing if the start/end is null , 
		{
			throw new  NoSuchElementException ();
		}
		
		if(start.equals(end))   //if the start and end are the same vertex then basically the distance is 0
		{
			return new Path(vertices.get(start));
		}
		PriorityQueue<Path> pq = new PriorityQueue<>(); //creating min priority queue 
		Set<NodeType> visited = new HashSet<>(); //hash set to store the visited ones 
		Path p = new Path(vertices.get(start)); //initially we will start with the path where end and start vertex are same 

		pq.add(p); //adding the above path to the priority queue 
		Path shortestpath = null; //creating a variable of type Path 
		while(!pq.isEmpty()) // looping through the queue until its empty 
		{
			Path temp = pq.remove() ; //removing the head from queue 
			visited.add(temp.end.data); //adding it to the visited set 
			List<Edge> list =  temp.end.edgesLeaving; //creating a list of type edge through that we will iterate 
			for(Edge e : list) //iterating 
			{
				Path temp2 = new Path(temp , e); 
				if(visited.contains(e.target.data)) //if set contains that specific target vertex then it means its visited 
				{
					continue;
				}
				
				pq.add(temp2); //adding that path to pq 
				
				if(e.target.data.equals(end)) //this means that we have reached end of path 
				{
					if(shortestpath == null) //if the path is null then we basically assign the value of path calculated 
					{
						shortestpath = temp2;
					
					}

					else if(temp2.compareTo(shortestpath) < 0 ) //compare the distances of both paths 
					{
						shortestpath = temp2; // assign the shorter one to shortestpath
					}
				}
			}
		}
		if(shortestpath == null) //last check to see if the path that has been calculated is null or not 
		{
			throw new NoSuchElementException(); //if null then throw exception
		}
		return shortestpath; //returning the shortest path 
	}
